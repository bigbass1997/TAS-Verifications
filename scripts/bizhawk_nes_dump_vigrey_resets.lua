...
while true do
    if dumpfile then
        -- Band-aid workaround patch
        if (emu.framecount() ~= movie.length() and movie.mode() ~= "PLAY" and dumpfile) then
            -- End Band-aid workaround patch
            dumpfile:close()
            dumpfile = nil
            print("dumping completed")
        -- Band-aid workaround patch
        elseif ((emu.framecount() ~= 0) or emu.framecount() == movie.length()) then
